...
function DogSearchList({ userData, refetchUser }) {
  ...
  const breeds = [
    ...new Map(filteredDogData?.map((dog) => [dog.breed, dog])).values(),
  ];

  const ages = [
    ...new Map(filteredDogData?.map((dog) => [dog.age, dog])).values(),
  ];

  const genders = [
    ...new Map(filteredDogData?.map((dog) => [dog.gender, dog])).values(),
  ];
